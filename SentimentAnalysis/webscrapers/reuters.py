...
def fetch_data(keyword):
    res = requests.get(link.format(keyword=keyword, count=10))
    if(res.ok):
...
def parse_data(res):
    ...
    ls = [{"Source": "https://www.reuters.com/", "title": i["title"], "description": i["description"], "url": ("https://www.reuters.com" + i["canonical_url"]), "image":i["thumbnail"]["renditions"]["original"]["480w"], "date": i["published_time"]} for i in data]
    df = pd.DataFrame(ls)
